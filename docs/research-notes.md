...
### 第三轮落地后的实测（同一条 39 秒翡翠口播，同一 brief）

| 版本 | 审片 | 主要问题 |
| --- | --- | --- |
| v8（第二轮规则） | fix 2/5 | 近景上贴小卡遮领口；花字蓝/白/红三色；黄字幕压在白色印花上；`above_head` 花字压到颈胸 |
| v9（第三轮，首轮） | fix 3/5 | 蓝色“知识-花字”预设与黄色高亮打架；底条 alpha 0.45 太淡，印花透出来；fix 轮误评了首轮成片（已修） |
| v10（第三轮 + 修正） | **ship 4/5**，无需修订轮 | 剩余两条为轻微：花字在推镜开始时仍在停留、两种花字颜色“轻微不一致”（白 + 强调色属设计内） |

v10 的开拍前检查自动得到人脸框 `{x:0.25,y:0.24,w:0.49,h:0.33}`（tight）与“胸口白色字母印花位于字幕区”，导演据此选了两处 `pip_face`、`chest` 花字，lint 把底条从 0.45 抬到 0.6；审片承诺核对 8/8 项可见。
