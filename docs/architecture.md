...
1. `getWordTimeline` 按优先级选源：内联 `words` → 外部对照接口 → Groq Whisper（把文案当 prompt 提升识别）。
...
- **catalog**：只包含 `get_*_types` 中 `cloud_render_supported: true` 的名字和官方花字 ID 表的子集，每项带“何时用”的提示，直接渲染进 prompt。
- **schema**：AI 输出结构。所有时间用片段 id 引用；beat 类型 punch/zoom/broll/effect 的条件必填由 `validatePlan` 补充检查。
...
- **lint**：高亮必须是字幕子串；字幕不压脸；同类 beat 不重叠；每分钟密度上限（punch 8 / broll 4 / effect 3，超出按时间均匀抽稀）；punch 与全屏 B-roll 同时出现时上移到顶部；按时间排序。所有修正都返回 warnings。
...
```
create_draft → add_video → [add_audio] → add_video_keyframe(所有 zoom 合并成一次调用)
...
- `VectCutClient`：Bearer 鉴权、5xx 重试、`success:false` 转异常、异步任务轮询（生图、渲染）、临时文件直传 OSS。
...
`npm test` 运行 31 个 `node:test` 用例：切片器、布局、schema/lint/planner 修复循环、编译器参数、执行器降级、LLM 结构化输出降级、VectCut 客户端错误处理、生图 provider、外部 ASR 适配、mock 端到端。全部离线。
