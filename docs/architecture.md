...
真实接口验证（2026-09，27s 口播）：Groq Whisper 5.3s 返回 137 词，字符级对齐覆盖 98%（VectCut sta 兜底路径同样跑通，切片时间差 ≤0.1s）；Gemini（原生 `generateContent` + `responseSchema`）一次通过校验；草稿含 22 条字幕、4 个花字、2 张生图 B-roll、1 个特效、BGM（草稿内 `volume 0.120`）与 4 条音效；云渲染成功。
